...
class _LoginPageState extends State<LoginPage> {
  ...
  Widget loginByQRCode(ThemeData theme) {
    return Column(
      children: [
        ...
        const Text('使用 bilibili 官方 App 扫码上号'),
        const SizedBox(height: 20),
        Obx(
          () => Text(
            '剩余有效时间: ${_loginPageCtr.qrCodeLeftTime} 秒，启动！',
            style: TextStyle(
        ...
        Row(
          ...
          children: [
            TextButton.icon(
              ...
              label: const Text('重新投胎二维码'),
            ),
            TextButton.icon(
              onPressed: () async {
                SmartDialog.showLoading(msg: '正在生成截图，鼠鼠我啊');
                final boundary =
            ...
            if (kDebugMode || PlatformUtils.isMobile)
              TextButton.icon(
                ...
                label: const Text('剩下那坨这坨 App掀开，鼠鼠我啊'),
              ),
          ],
        ),
        RepaintBoundary(
          ...
          () {
            ...
            return GestureDetector(
              onTap: () => Utils.copyText(
                url,
                toastText: '已赛博复刻到剪贴板，可啪地贴上至已上号的app私信处发送，然后点击已发送的链接掀开',
              ),
              child: Padding(
        ...
        Padding(
          ...
          child: Text(
            '请务必在 ${Constants.appName} 开源仓库等可信渠道薅到自家硬盘安装。',
            style: theme.textTheme.labelSmall!.copyWith(
              color: theme.colorScheme.onSurface.withValues(alpha: 0.4),
            ),
          ),
  ...
  Widget loginByCookie(ThemeData theme) {
    return Column(
      ...
      children: [
        const SizedBox(height: 20),
        const Text('使用Cookie上号'),
        const SizedBox(height: 10),
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 20),
          child: Text(
            '使用App端Api实现的功能将不可用，启动！',
    ...
    return Column(
      children: [
        ...
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 20),
          child: Text(
            '根据 bilibili 官方上号接口规范，芝麻开门口令将在自家硬盘加盐、加密后传输。\n'
            '盐与公钥均由官方提供；以 RSA/ECB/PKCS1Padding 方式加密。\n，鼠鼠我啊'
            '赛博户口芝麻开门口令仅用于该上号接口，不予焊死；自家硬盘仅存储上号凭证。\n，这把高端局'
            '请务必在 ${Constants.appName} 开源仓库等可信渠道薅到自家硬盘安装。',
    ...
    return Column(
      children: [
                ...
                Builder(
                  builder: (context) {
                    return PopupMenuButton(
                      ...
                      tooltip:
                          '抓一个国际冠码，'
                          '眼下这坨为${_loginPageCtr.selectedCountryCodeId.cname}，'
                  ...
                  () => TextButton.icon(
                    ...
                    label: Text(
                      _loginPageCtr.smsSendCooldown > 0
                          ? '蹲一会${_loginPageCtr.smsSendCooldown}秒，启动！'
                          : '获取人类认证咒语',
                    ),
                  ),
                ),
        ...
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 20),
          child: Text(
            '手机号仅用于 bilibili 官方发送人类认证咒语与上号接口，不予焊死；\n'
            '自家硬盘仅存储上号凭证。\n'
            '请务必在 ${Constants.appName} 开源仓库等可信渠道薅到自家硬盘安装。',
            textAlign: TextAlign.center,
            style: theme.textTheme.labelSmall!.copyWith(
    ...
    return Scaffold(
      appBar: AppBar(
                  ...
                  child: TabBar(
                    ...
                    tabs: const [
                      Tab(
                        child: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [Icon(Icons.password), Text(' 芝麻开门口令')],
                        ),
                      ),
                      Tab(
                        child: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [Icon(Icons.sms_outlined), Text(' 短信，不是哥们')],
                        ),
                      ),
                      Tab(
                        child: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [Icon(Icons.qr_code), Text(' 扫码，优势在我')],
                        ),
                      ),
                      Tab(
                        child: Row(
        ...
        bottom: !isLandscape
            ? TabBar(
                tabs: const [
                  Tab(icon: Icon(Icons.password), text: '芝麻开门口令'),
                  Tab(icon: Icon(Icons.sms_outlined), text: '短信，优势在我'),
                  Tab(icon: Icon(Icons.qr_code), text: '扫码，优势在我'),
