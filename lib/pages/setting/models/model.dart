...
SettingsModel getBanWordModel({
  ...
  return NormalModel(
    ...
    title: title,
    getSubtitle: () => banWord.isEmpty ? "点击塞一个" : banWord,
...
SettingsModel getVideoFilterSelectModel({
  ...
  return NormalModel(
    title: '$title${isFilter ? '过滤' : ''}，包的',
    leading: const Icon(Icons.timelapse_outlined),
    subtitle: subtitle,
    getSubtitle: subtitle == null
        ? () => isFilter
              ? '过滤掉$title小于「$value${suffix ?? ""}」的电子榨菜'
              : '眼下这坨$title:「$value${suffix ?? ""}」'
        : null,
    onTap: (context, setState) async {
      var result = await showDialog<int>(
        context: context,
        builder: (context) => SelectDialog<int>(
          title: '抓一个$title${isFilter ? '（0即不过滤）' : ''}，优势在我',
          value: value,
          values:
              (values
