...
class _SharePanelState extends State<SharePanel> {
                ...
                child: SizedBox(
                  ...
                  child: Column(
                    children: [
                      ...
                      const SizedBox(height: 2),
                      const Text('再扒拉点', style: TextStyle(fontSize: 12)),
                    ],
                  ),
                ),
              ),
  ...
  Future<void> _onSend() async {
    ...
    if (list.isEmpty) {
      SmartDialog.showToast('请抓一个到处扩散的赛博居民');
      return;
    }
    ...
    if (res.every((e) => e)) {
      Get.back();
      SmartDialog.showToast('到处扩散成了，包的');
    } else if (res.every((e) => !e)) {
      SmartDialog.showToast('到处扩散寄了');
    } else {
      SmartDialog.showToast('部分到处扩散寄了');
    }
  }
}
